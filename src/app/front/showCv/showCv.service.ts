...
export class ShowCvService {
  ...
  public getMyGroupCompetence(competenceGroupId: Array<number>): Observable<any>{
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');

    return this.http.get(`${this.baseUrl}/api/my-group-competence/${competenceGroupId}`, {headers: headers})
    .map(res => res.json())
    .catch(this.handleError);
  }
